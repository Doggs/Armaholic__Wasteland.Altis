...
switch (_lockState) do
{
    case 0: // LOCK
	{
		...
		_checks =
		{
			...
			switch (true) do
			{
				...
				case (doCancelAction): { doCancelAction = false; _text = "Anchoring cancelled" };
				...
				case (_object getVariable ["objectLocked", false]): { _text = "Somebody else anchored it before you" };
				default
				{
					_failed = false;
					_text = format ["Anchoring %1%2 complete", floor (_progress * 100), "%"];
				};
			};
		...
		if (_success) then
		{
			...
			_object setVariable ["A3W_saveEnabled", true, true];
			if (_object getVariable ["A3W_isHouse", false]) then {
				_object setVariable ["A3W_allowDamage", false, true];
				_object allowDamage false;
			};			
			[format ["%1 Anchored. Saving ENABLED.", getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")], 4 ] call mf_notify_client;
		};
    ...
    case 1: // UNLOCK
	{
		...
		_checks =
		{
			...
			{
				case (!alive player): {};
				case (doCancelAction): { doCancelAction = false; _text = "Un-Anchoring cancelled" };
				case (vehicle player != player): { _text = "Action failed! You can't do this in a vehicle" };
				case (!isNull (_object getVariable ["R3F_LOG_est_transporte_par", objNull])): { _text = "Action failed! Somebody moved the object" };
				case !(_object getVariable ["objectLocked", false]): { _text = "Somebody else un-anchored it before you" };
				default
				{
					_failed = false;
					_text = format ["Un-Anchoring %1%2 complete", floor (_progress * 100), "%"];
				};
			};
		...
		if (_success) then
		{
			...
			_object setVariable ["A3W_saveEnabled", false, true];
			_object setVariable ["A3W_allowDamage", true, true];
			_object allowDamage true;
			if (!(_object getVariable ["A3W_isHouse", false])) then {
				_object setVariable ["ownerUID", nil, true];
			};
			[format ["%1 Un-Anchored.  Saving DISABLED.", getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")], 4 ] call mf_notify_client;
		};
