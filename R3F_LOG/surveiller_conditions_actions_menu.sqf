...
//	systemChat format ["OwnerID: %1", _objet_pointe getVariable "ownerUID"];
//	systemChat format ["R3F_disabled: %1", _objet_pointe getVariable "R3F_LOG_disabled"];
//	systemChat format ["objectLocked: %1", _objet_pointe getVariable "objectLocked"];
	...
	) then
	{
		...
		if ({_objet_pointe isKindOf _x} count R3F_LOG_CFG_can_be_moved_by_player > 0) then
		{
			...
			R3F_LOG_action_deplacer_objet_valide = (count crew _objet_pointe == 0) && (isNull R3F_LOG_joueur_deplace_objet) &&
				...
				!(_objet_pointe getVariable "R3F_LOG_disabled") && !(_objet_pointe getVariable ["objectLocked", false]) && 
				(!(_objet_pointe getVariable ['A3W_isHouse', false]) || _objet_pointe getVariable ['ownerUID', ''] == getPlayerUID player);
		};
		
		// Condition action lock object
		if ({_objet_pointe isKindOf _x} count R3F_LOG_CFG_can_be_locked_by_player > 0) then
		{
			Object_isLocked = (_objet_pointe getVariable ["objectLocked", false]);
			R3F_LOG_action_lock_object_valid = !(_objet_pointe getVariable ["R3F_LOG_disabled", false]);
		};
		...
		if ({_objet_pointe isKindOf _x} count R3F_LOG_classes_objets_transportables > 0) then
		{
			...
			if ({_objet_pointe isKindOf _x} count R3F_LOG_CFG_can_be_moved_by_player > 0) then
			{
				...
				R3F_LOG_action_charger_deplace_valide = (count crew _objet_pointe == 0) && (R3F_LOG_joueur_deplace_objet == _objet_pointe) &&
					!(_objet_pointe getVariable "R3F_LOG_disabled") && !isNull _objet_pointe_autre_que_deplace && !(_objet_pointe getVariable ["objectLocked", false]) &&
					(!(_objet_pointe getVariable ["A3W_isHouse", false]) || _objet_pointe getVariable ["ownerUID", ""] == getPlayerUID player) &&
					{
			...
			R3F_LOG_action_selectionner_objet_charge_valide = (count crew _objet_pointe == 0) && isNull R3F_LOG_joueur_deplace_objet &&
				...
				_objet_pas_en_cours_de_deplacement && !(_objet_pointe getVariable "R3F_LOG_disabled") && !(_objet_pointe getVariable ["objectLocked", false]) && 
				(!(_objet_pointe getVariable ["A3W_isHouse", false]) || _objet_pointe getVariable ["ownerUID", ""] == getPlayerUID player);
		};
		...
		if ({_objet_pointe isKindOf _x} count R3F_LOG_classes_transporteurs > 0) then
		{
			...
			R3F_LOG_action_charger_deplace_valide = alive _objet_pointe && (!isNull R3F_LOG_joueur_deplace_objet) &&
				...
				!(_objet_pointe getVariable ["objectLocked", false]);
