...
if ({_objet isKindOf _x} count R3F_LOG_CFG_can_be_locked_by_player > 0) then
{
	_objet addAction [("<img image='client\icons\r3f_lock.paa' color='#E01616'/> <t color='#f5f5f5'>" + format [STR_LOCK_OBJECT, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")] + "</t>"), {_this call R3F_LOG_FNCT_object_lsm}, 0, -5, false, true, "", "!R3F_LOG_mutex_local_verrou && R3F_LOG_objet_addAction == _target && R3F_LOG_action_lock_object_valid && !Object_isLocked && (!(_target isKindOf 'AllVehicles') || {_target isKindOf 'StaticWeapon'}) && (!(_target getVariable ['A3W_isHouse', false]) || _target getVariable ['ownerUID', ''] == getPlayerUID player)"];
	_objet addAction [("<img image='client\icons\r3f_unlock.paa' color='#64E060'/> <t color='#f5f5f5'>" + format [STR_UNLOCK_OBJECT, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")] + "</t>"), {_this call R3F_LOG_FNCT_object_lsm}, 1, -5, false, true, "", "!R3F_LOG_mutex_local_verrou && R3F_LOG_objet_addAction == _target && R3F_LOG_action_lock_object_valid && Object_isLocked && (!(_target getVariable ['A3W_isHouse', false]) || _target getVariable ['ownerUID', ''] == getPlayerUID player)"];
};
