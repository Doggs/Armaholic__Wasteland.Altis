...
while {true} do
{
	...
	_str = format["%1<br/>%2 <img size='0.6' image='client\icons\bank.paa'/>", _str, [player getVariable ["bmoney", 0]] call fn_numbersText];
	_str = format["%1<br/>%2 <img size='0.6' image='client\icons\money.paa'/>", _str, [player getVariable ["cmoney", 0]] call fn_numbersText];
	_str = format["%1<br/>%2 <img size='0.6' image='client\icons\water.paa'/>", _str, ceil (thirstLevel max 0)];
	_str = format["%1<br/>%2 <img size='0.6' image='client\icons\food.paa'/>", _str, ceil (hungerLevel max 0)];
	_str = format["%1<br/><t color='%2'>%3</t> <img size='0.6' image='client\icons\health.paa'/>", _str, _healthTextColor, _health];
